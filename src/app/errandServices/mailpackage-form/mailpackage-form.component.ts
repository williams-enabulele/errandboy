...
import { MailpackageService } from './mailpackage.service';
...
@Component({
  selector: 'app-mailpackage-form',
  templateUrl: './mailpackage-form.component.html',
  styleUrls: ['./mailpackage-form.component.scss']
})
export class MailpackageFormComponent implements OnInit {

  showbutton = true;
  showbtnloading = false;

  mailpackageForm: FormGroup;
 ...
 */
  ...
  ngOnInit(): void {
    ...
    this.service = null;
    this.mailpackageForm = this.fb.group({
  ...
  get f() { return this.mailpackageForm.controls; }
    ...
    results.subscribe(res => {
      ...
      this.pricingService.get_pricing(this.errandService.nativeElement.value, this.location_distance).subscribe(
        price => {
          ...
          this.form = { order_no: this.order_no, service_id: 3, service_errand: "Mail | Package Pickup and Drop-off", cost: this.service_fee, location_distance: this.l_distance, destinationAddress: this.destinationAddress.nativeElement.value, originAddress: this.originAddress.nativeElement.value, duration: this.travel_duration,fromno: this.fromno, tono: this.tono  };
          //console.log(price);
        }
      )
  ...
  onSubmit() {
    ...
    ////console.log(this.mailpackageForm.value);
    this.formvalues = { ...this.form, ...this.mailpackageForm.value };
    //console.log(this.formvalues);
    this.isSubmitted = true;
    this.billOrder();
    if (this.mailpackageForm.invalid) {
      return;
    }
