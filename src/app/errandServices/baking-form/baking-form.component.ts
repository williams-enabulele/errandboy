...
@Component({
  selector: 'app-baking-form',
  templateUrl: './baking-form.component.html',
  styleUrls: ['./baking-form.component.scss']
})
export class BakingFormComponent implements OnInit {
 ...
 */
  ...
  get f() { return this.bakingForm.controls; }
    ...
    results.subscribe(res => {
      ...
      this.pricingService.get_pricing(this.errandService.nativeElement.value, this.location_distance).subscribe(
        price => {
          ...
          this.form = { order_no: this.order_no, service_id: "4", service_errand: "Baking Pickup and Delivery", cost: this.service_fee, location_distance: this.l_distance, destinationAddress: this.destinationAddress.nativeElement.value, originAddress: this.originAddress.nativeElement.value, duration: this.travel_duration, fromno: this.fromno, tono: this.tono};
          //console.log(price);
        }
      )
    },
  ...
  onSubmit() {
    ...
    this.formvalues = { ...this.form, ...this.bakingForm.value };
    //console.log(this.formvalues);
    this.isSubmitted = true;
    this.billOrder();
    if (this.bakingForm.invalid) {
      return;
    }
  }
